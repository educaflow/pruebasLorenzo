...
data class GreaterThanIf<T : Comparable<T>>(val comparableAnotherField:KFunction<T?>,val dependentField:KFunction<*>,val dependentValue: Any?) : ValidationRule {
    override fun validate(value: Any?, bean: Any): String? {
    ...
    data class GreaterThanOrEqualIf<T : Comparable<T>>(val comparableAnotherField: KFunction<T?>, val dependentField: KFunction<*>, val dependentValue: Any?) : ValidationRule {
        override fun validate(value: Any?, bean: Any): String? {
            val currentDependentValue = dependentField.call(bean)
            if (currentDependentValue != dependentValue) return null

            val anotherValue = comparableAnotherField.call(bean) ?: return null
            if (value == null) return null

            if (value is Comparable<*>) {
                @Suppress("UNCHECKED_CAST") val comparableValue = value as T
                return if (comparableValue.compareTo(anotherValue) >= 0) null
                else "El valor debe ser mayor o igual que $anotherValue"
            }
            return null
    ...
    data class LessThanIf<T : Comparable<T>>(val comparableAnotherField: KFunction<T?>, val dependentField: KFunction<*>, val dependentValue: Any?) : ValidationRule {
        override fun validate(value: Any?, bean: Any): String? {
            val currentDependentValue = dependentField.call(bean)
            if (currentDependentValue != dependentValue) return null
    ...
    data class LessThanOrEqualIf<T : Comparable<T>>(val comparableAnotherField: KFunction<T?>, val dependentField: KFunction<*>, val dependentValue: Any?) : ValidationRule {
        override fun validate(value: Any?, bean: Any): String? {
            val currentDependentValue = dependentField.call(bean)
            if (currentDependentValue != dependentValue) return null
    ...
    data class EqualToIf<T>(val anotherField: KFunction<T?>, val dependentField: KFunction<*>, val dependentValue: Any?) : ValidationRule {
        override fun validate(value: Any?, bean: Any): String? {
            val currentDependentValue = dependentField.call(bean)
            if (currentDependentValue != dependentValue) return null
    ...
    data class NotEqualToIf<T>(val anotherField: KFunction<T?>, val dependentField: KFunction<*>, val dependentValue: Any?) : ValidationRule {
        override fun validate(value: Any?, bean: Any): String? {
            val currentDependentValue = dependentField.call(bean)
            if (currentDependentValue != dependentValue) return null
